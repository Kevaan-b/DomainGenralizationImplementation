...
def stratified_source_split(labels: np.ndarray, seed: int, budget: float = 1.0) -> SplitIndices:
    ...
    for label in np.unique(labels):
        ...
        validation_count = max(1, int(round(selected_count * 0.1))) if selected_count > 1 else 0
